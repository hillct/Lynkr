...
if (Database) {
  try {
    ...
    logger.warn({ err: err.message }, "better-sqlite3 not available, running without database");
    db = null;
  }
} else {
  logger.info("better-sqlite3 not available, running without database");
}

// When db is unavailable, export a stub that provides no-op methods
// so consumers don't crash on db.prepare(), db.exec(), etc.
if (!db) {
  const noopStmt = {
